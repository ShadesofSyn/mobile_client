...
func set_player_whitened() -> void:
	...
	sprite_material.call_deferred("set_shader_parameter","flash_color",Color("ffffff"))
	sprite_material.call_deferred("set_shader_parameter", "flash_modifier", 0.7)
	await get_tree().create_timer(0.5).timeout
	sprite_material.call_deferred("set_shader_parameter", "flash_modifier", 0.0)
...
func _on_timer_timeout():
	...
	ghost.position = sprite.position + get_parent().global_position
	Server.world.projectiles.call_deferred("add_child", ghost)
